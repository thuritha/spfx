...
export class spOperation {
    ...
    public getOrderList(context: WebPartContext): Promise<IDropdownOption[]> {
        ...
        let restApiurl: string = context.pageContext.web.absoluteUrl + "/_api/web/lists/getbytitle('Orders')/items?select=OrderID";
        return new Promise<IDropdownOption[]>(async (resolve, reject) => {
            context.spHttpClient
                .get(restApiurl, SPHttpClient.configurations.v1)
                .then((response: SPHttpClientResponse) => {
                ...
                .then((response: SPHttpClientResponse) => {
                    ...
                    if (response.ok) {
                        response.json().then(
                            (result: any) => {
                                // console.log(result);
                                resolve("Order with ID: " + result.OrderID + " created Successfully!");
                            },
                            (error: any): void => {
    ...
    public getUpdateitem(context: WebPartContext, data: any): Promise<any> {
        ...
        let restApiUrl: string =
            context.pageContext.web.absoluteUrl +
            "/_api/web/lists/getbytitle('Orders')/items?$filter=(OrderID eq " + data.text + ")";
    ...
    public getProductDetails(context: WebPartContext, data: any) {
        let restApiUrl: string =
            context.pageContext.web.absoluteUrl +
            "/_api/web/lists/getbytitle('Products')/items?$filter=(OrderID eq " +
            data.key +
            ") and (ProductName eq '" +
            data.text +
            "')";
